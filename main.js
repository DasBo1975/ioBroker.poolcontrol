...
} = require("./lib/stateDefinitions/temperatureStates");
...
} = require("./lib/stateDefinitions/consumptionStates");
...
class Poolcontrol extends utils.Adapter {
  ...
  onStateChange(id, state) {
    ...
    try {
      ...
      this.log.warn(
        `[temperatureHelper] Fehler in handleStateChange: ${e.message}`,
      );
    }
    try {
      ...
      this.log.warn(
        `[runtimeHelper] Fehler in handleStateChange: ${e.message}`,
      );
    }
    try {
    ...
    try {
      ...
      this.log.warn(
        `[consumptionHelper] Fehler in handleStateChange: ${e.message}`,
      );
    }
  }
}
