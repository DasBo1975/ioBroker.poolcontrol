...
async function createTemperatureStates(adapter) {
  ...
  async function createSensorSet(sensorKey, label) {
    ...
    await adapter.setObjectNotExistsAsync(
      `temperature.${sensorKey}_temp_active`,
      {
        type: "state",
        common: {
    ...
    await adapter.setObjectNotExistsAsync(
      `temperature.${sensorKey}_temp_sensor`,
      {
        type: "state",
        common: {
    ...
    await adapter.setObjectNotExistsAsync(
      `temperature.${sensorKey}.min_today`,
      {
        type: "state",
        common: {
    ...
    await adapter.setObjectNotExistsAsync(
      `temperature.${sensorKey}.max_today`,
      {
        type: "state",
        common: {
    ...
    await adapter.setObjectNotExistsAsync(
      `temperature.${sensorKey}.delta_per_hour`,
      {
        type: "state",
        common: {
