...
async function createGeneralStates(adapter) {
    ...
    await adapter.setStateAsync("general.pool_name", { val: adapter.config.pool_name, ack: true });
    ...
    await adapter.setStateAsync("general.min_circulation_per_day", { val: adapter.config.min_circulation_per_day, ack: true });
    ...
    await adapter.setStateAsync("general.pool_size", { val: adapter.config.pool_size, ack: true });
}
