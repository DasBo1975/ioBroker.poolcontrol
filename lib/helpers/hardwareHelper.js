...
class HardwareHelper {
    ...
    async _searchAndHandshake(boxId) {
        ...
        try {
            ...
            // Schreibe Antwort in den passenden Rückkanal (gleiche Struktur, aber "handshake_confirm" oder TextSensor)
            const confirmPath = foundState.replace(/\.state$/, '.handshake_confirm');
            if (await adapter.getObjectAsync(confirmPath)) {
