...
class HardwareHelper {
    ...
    constructor(adapter) {
        ...
        this.scanInterval = null; // FIX: damit Timer gestoppt werden kann
    }
    ...
    async init() {
        ...
        // ------------------------------------------------------
        // FIX: Listener für Checkbox muss IMMER aktiv sein, damit spätere Aktivierung erkannt wird
        // ------------------------------------------------------
        try {
            adapter.subscribeStates('hardware.tempbox.settings.enabled');
            adapter.on('stateChange', async (id, state) => {
                ...
                if (
                    ...
                    if (!enabled) {
                        await this._setBlueLed(false);
                        await adapter.setStateAsync('hardware.tempbox.status.connected', { val: false, ack: true });
                        await adapter.setStateAsync('hardware.tempbox.handshake.state', { val: 'disabled', ack: true });
                        adapter.log.info('[hardwareHelper] TempBox deaktiviert – Status-LED (blau) ausgeschaltet.');
                        await adapter.setStateAsync('hardware.text_message', {
                            val: 'Temperatur-Box (ESP32): ❌ deaktiviert | ',
                            ack: true,
                        });
                        // FIX: Laufende Scans/TMR stoppen
                        if (this.scanInterval) {
                            clearInterval(this.scanInterval);
                            this.scanInterval = null;
                            adapter.log.info('[hardwareHelper] Regelmäßige Scans gestoppt (Box deaktiviert).');
                        }
                    } else {
                        adapter.log.info('[hardwareHelper] TempBox aktiviert – starte Handshake-Suche.');
                        await this._performFullScan();
                        this._scheduleRegularScan(); // FIX: Timer erst jetzt aktivieren
                    }
                }
            });
        } catch (err) {
            adapter.log.warn(`[hardwareHelper] Konnte Checkbox-Listener nicht setzen: ${err.message}`);
        }

        // ------------------------------------------------------
        // FIX: nur starten, wenn Checkbox aktiv ist
        // ------------------------------------------------------
        try {
            ...
            if (!enabled) {
                ...
                return; // <-- beendet Init, aber Listener bleibt aktiv
            }
        } catch (err) {
    ...
    async _performFullScan() {
        ...
        try {
            ...
            if (!enabled) {
                ...
                await adapter.setStateAsync('hardware.tempbox.status.connected', { val: false, ack: true });
                await adapter.setStateAsync('hardware.tempbox.handshake.state', { val: 'disabled', ack: true });
                textMessage += 'Temperatur-Box (ESP32): ❌ deaktiviert | ';
                adapter.log.info(`[hardwareHelper] Scan abgebrochen – Box deaktiviert.`);
        ...
        // Blaue LED anhand des Ergebnisses setzen
        try {
        ...
        if (manualPath) {
            ...
            adapter.on('stateChange', async (id, state) => {
                if (id === manualPath && state && typeof state.val === 'string' && state.val.startsWith(code)) {
                    ...
                    // Device-Map dynamisch über source_path aufbauen
                    try {
                        ...
                        for (const row of objects.rows) {
                            ...
                            // LED-Erkennung (Grün, Blau, Rot – korrigiert auf .state)
                        ...
                        // Blaue LED sofort nach erfolgreichem Handshake einschalten
                        try {
                            if (deviceMap.led_blue_path) {
                                await adapter.setForeignStateAsync(deviceMap.led_blue_path, { val: true, ack: false });
                                adapter.log.info(
                    ...
                    // Handshake-Bestätigung an ESPHome senden
                    try {
    ...
    cleanup() {
        const { adapter } = this;
        if (this.scanInterval) {
            clearInterval(this.scanInterval); // FIX: sicherstellen, dass Timer gestoppt wird
        }
    ...
    async _setBlueLed(on) {
        const { adapter } = this;
        try {
            const mapState = await adapter.getStateAsync('hardware.tempbox.device_map');
            const deviceMap = mapState?.val ? JSON.parse(mapState.val) : {};
            const bluePath = deviceMap?.led_blue_path;
            if (bluePath) {
                await adapter.setForeignStateAsync(bluePath, { val: !!on, ack: false });
            }
        } catch (err) {
            adapter.log.warn(`[hardwareHelper] _setBlueLed fehlgeschlagen: ${err.message}`);
        }
    }
}
