...
class HardwareHelper {
    ...
    async _searchAndHandshake(boxId) {
        ...
        // NEU: Direkte Pfadverwendung aus Instanz-Konfiguration (statt Vollsuche)
        let manualPath = '';
        if (boxId === 'tempbox') {
            manualPath = adapter.config.tempbox_statepath || '';
        } else if (boxId === 'tasterbox') {
            manualPath = adapter.config.tasterbox_statepath || '';
        }
        ...
        if (manualPath) {
            ...
            try {
                const stateObj = await adapter.getStateAsync(manualPath);
                adapter.log.debug(`[hardwareHelper] Prüfe Wert im Pfad: "${stateObj?.val}" gegen Code "${code}"`);
                if (stateObj && typeof stateObj.val === 'string' && stateObj.val.startsWith(code)) {
                    const handshakeValue = stateObj.val;
                    ...
                    adapter.log.info(
                        `[hardwareHelper] ${BOX_TYPES[boxId].label} erfolgreich erkannt (Pfad aus Config).`,
                    );
                    return true; // überspringt die alte Suchroutine
                }
                adapter.log.warn(
                    `[hardwareHelper] ${BOX_TYPES[boxId].label}: Angegebener Pfad ist ungültig oder enthält keine Box-Kennung.`,
                );
            } catch (err) {
                adapter.log.warn(`[hardwareHelper] Fehler beim Lesen des konfigurierten Pfads: ${err.message}`);
            }
        }
        ...
        try {
            // FIX: Keine system-View mehr – stattdessen Live-Objektliste unter esphome.0.*
            ...
            // FIX: Suche über den common.name der Box-ID (stabil auch bei kryptischen IDs)
            for (const obj of objects.rows) {
                ...
                const commonName = (obj?.value?.common?.name || '').toString().toLowerCase();
                // Exakte Namenssuche für die TempBox
                // (Einzigartiger Common-Name aus ESPHome: "State of PoolControl Box-ID")
                const isBoxIdName = commonName.trim() === 'state of poolcontrol box-id';
            ...
            // FIX: Namensbasierte Suche nach Handshake-Bestätigung (falls kein .handshake_confirm existiert)
            ...
            if (!confirmObj) {
                const alt = objects.rows.find(
                    o =>
                        ...
                        (o.value?.common?.name || '').toString().toLowerCase().includes('handshake-best'),
                );
                if (alt) {
                    confirmPath = alt.id;
