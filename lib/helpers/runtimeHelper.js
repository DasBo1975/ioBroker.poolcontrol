...
const runtimeHelper = {
  ...
  async _updateStates() {
    try {
      ...
      const pumpLph =
        (await this.adapter.getStateAsync("pump.pump_power_lph"))?.val || 0;
      const poolSize =
        (await this.adapter.getStateAsync("general.pool_size"))?.val || 0;
      const minCirc =
        (await this.adapter.getStateAsync("general.min_circulation_per_day"))
          ?.val || 1;
      ...
      this.adapter.log.warn(
        `[runtimeHelper] Fehler beim Update der States: ${err.message}`,
      );
    }
  },
  ...
  _startLiveTimer() {
    ...
    this.adapter.log.debug(
      "[runtimeHelper] Live-Timer gestartet (Updates jede Minute)",
    );
  },
