...
const frostHelper = {
  ...
  async _checkFrost() {
    try {
      ...
      const frostActive = (
        await this.adapter.getStateAsync("pump.frost_protection_active")
      )?.val;
      if (!frostActive) {
        return;
      }
      ...
      const frostTemp = (
        await this.adapter.getStateAsync("pump.frost_protection_temp")
      )?.val;
      if (frostTemp == null) {
        return;
      }
      ...
      const outside = (
        await this.adapter.getStateAsync("temperature.outside.current")
      )?.val;
      if (outside == null) {
      ...
      const pumpActive = (await this.adapter.getStateAsync("pump.pump_switch"))
        ?.val;
