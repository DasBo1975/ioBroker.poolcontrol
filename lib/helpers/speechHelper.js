...
const speechHelper = {
    ...
    async handleStateChange(id, state) {
        ...
        if (id.endsWith("pump.pump_switch")) {
            if (state.val) {
                const txt = (await this.adapter.getStateAsync("speech.start_text"))?.val || "Die Poolpumpe wurde gestartet.";
                await this._speak(txt);
            } else {
                const txt = (await this.adapter.getStateAsync("speech.end_text"))?.val || "Die Poolpumpe wurde gestoppt.";
                await this._speak(txt);
            }
            return;
        }
