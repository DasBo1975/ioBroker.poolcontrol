...
const speechHelper = {
  ...
  async handleStateChange(id, state) {
    ...
    if (id.endsWith("pump.pump_switch")) {
      if (state.val) {
        ...
        const txt =
          (await this.adapter.getStateAsync("speech.end_text"))?.val ||
          "Die Poolpumpe wurde gestoppt.";
        await this._speak(txt);
      }
      return;
    }
  ...
  async _speak(text) {
    try {
      ...
      // Alexa-Ausgabe
      if (
        this.adapter.config.speech_alexa_enabled &&
        this.adapter.config.speech_alexa_device
      ) {
      ...
      // Telegram-Ausgabe
      if (
        this.adapter.config.speech_telegram_enabled &&
        this.adapter.config.speech_telegram_instance
      ) {
      ...
      this.adapter.log.warn(
        `[speechHelper] Fehler beim Sprechen: ${err.message}`,
      );
    }
  },
