...
const speechHelper = {
    adapter: null,
    lastTempNotify: {}, // <--- NEU: Cooldown-Speicher pro Sensor
            ...
            if (val >= threshold && threshold > 0) {
                ...
                if (now - last > 60 * 60 * 1000) {
                    ...
                    this.adapter.log.debug(
                        `[speechHelper] Temperaturansage für ${id} unterdrückt (Cooldown aktiv).`
                    );
                }
            }
            return;
    ...
    async _speak(text) {
        try {
            ...
            if (this.adapter.config.speech_email_enabled && this.adapter.config.speech_email_instance) {
                ...
                // --- NEU: Existenzprüfung ---
                const obj = await this.adapter.getForeignObjectAsync(sendState);
                if (!obj) {
                    this.adapter.log.error(`[speechHelper] E-Mail-Instanz nicht gefunden: ${sendState}`);
                    return;
                }
