    ...
    try {
      ...
      if (!pumpSwitchId) {
        this.adapter.log.warn(
          "[timeHelper] Keine pump_switch (Fremd-ID) konfiguriert!",
        );
        return;
      }
      ...
      for (let i = 1; i <= 3; i++) {
        const active = (
          await this.adapter.getStateAsync(`timecontrol.time${i}_active`)
        )?.val;
        const start = (
          await this.adapter.getStateAsync(`timecontrol.time${i}_start`)
        )?.val;
        const end = (
          await this.adapter.getStateAsync(`timecontrol.time${i}_end`)
        )?.val;
        const dayOk = (
          await this.adapter.getStateAsync(
            `timecontrol.time${i}_day_${weekday}`,
          )
        )?.val;
      ...
      this.adapter.log.debug(
        `[timeHelper] Pumpe ${shouldRun ? "EIN" : "AUS"} (${hhmm})`,
      );
    } catch (err) {
