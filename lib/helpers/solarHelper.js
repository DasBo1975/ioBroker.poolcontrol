...
const solarHelper = {
  ...
  async _checkSolar() {
    try {
      ...
      const active = (
        await this.adapter.getStateAsync("solar.solar_control_active")
      )?.val;
      if (!active) {
        return;
      }
      ...
      const hysteresis = (
        await this.adapter.getStateAsync("solar.hysteresis_active")
      )?.val;

      // Temperaturen laden
      const collector = (
        await this.adapter.getStateAsync("temperature.collector.current")
      )?.val;
      const pool = (
        await this.adapter.getStateAsync("temperature.surface.current")
      )?.val; // Oberfläche = Pooltemp

      if (collector == null || pool == null) {
        this.adapter.log.debug(
          "[solarHelper] Keine gültigen Temperaturen verfügbar",
        );
        return;
      }
      ...
      // --- Kollektor-Warnung ---
      const warnActive = (await this.adapter.getStateAsync("solar.warn_active"))
        ?.val;
      if (warnActive) {
        const warnTemp =
          Number((await this.adapter.getStateAsync("solar.warn_temp"))?.val) ||
          0;
        const speechEnabled = (
          await this.adapter.getStateAsync("solar.warn_speech")
        )?.val;
        const currentWarning =
          (await this.adapter.getStateAsync("solar.collector_warning"))?.val ||
          false;
