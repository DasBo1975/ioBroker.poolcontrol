...
const solarHelper = {
    ...
    async _checkSolar() {
        try {
            ...
            await this.adapter.setStateAsync("pump.pump_switch", { val: shouldRun, ack: false });
            this.adapter.log.debug(`[solarHelper] Solarregelung → Pumpe ${shouldRun ? "EIN" : "AUS"} (Collector=${collector}°C, Pool=${pool}°C, Delta=${delta}°C)`);
            ...
            if (warnActive) {
                ...
                if (collector >= warnTemp && !currentWarning) {
                    await this.adapter.setStateAsync("solar.collector_warning", { val: true, ack: true });
                    this.adapter.log.warn(`[solarHelper] WARNUNG: Kollektortemperatur ${collector}°C >= ${warnTemp}°C!`);
                ...
                if (collector <= warnTemp * 0.9 && currentWarning) {
                    await this.adapter.setStateAsync("solar.collector_warning", { val: false, ack: true });
                    this.adapter.log.info(`[solarHelper] Kollektorwarnung zurückgesetzt: ${collector}°C <= ${warnTemp * 0.9}°C`);
                }
            }
