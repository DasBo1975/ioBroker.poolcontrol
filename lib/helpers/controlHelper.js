...
async function handleStateChange(id, state) {
    try {
        ...
        if (id.endsWith('control.energy.reset') && state.val === true) {
            ...
            for (const id of consStates) {
                await adapter.setStateAsync(id, { val: 0, ack: true });
            }

            await adapter.setStateAsync('control.energy.reset', { val: false, ack: true });

            adapter.log.info('[controlHelper] Energiezähler wurde erfolgreich zurückgesetzt.');

            if (notify) {
                await sendNotification(`Energiezähler wurde am ${timestamp} vollständig zurückgesetzt.`);
            }
        }
    } catch (err) {
