...
async function sendNotification(text) {
    ...
    try {
        ...
        // Telegram (modern über sendTo)
        if (adapter.config.speech_telegram_enabled && adapter.config.speech_telegram_instance) {
            const instance = adapter.config.speech_telegram_instance;
            try {
                ...
                adapter.log.warn(`[controlHelper] Telegram-Versand fehlgeschlagen (${instance}): ${err.message}`);
            }
        }
