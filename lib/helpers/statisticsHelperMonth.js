...
const statisticsHelperMonth = {
    ...
    async _resetMonthlyTemperatureStats() {
        ...
        // FIX: Monatsreset nur durchführen, wenn letztes Reset-Datum gültig ist
        try {
            const lastResetState = await adapter.getStateAsync('analytics.statistics.temperature.month.last_reset');
            const lastResetVal = lastResetState?.val;
            const now = new Date();

            if (!lastResetVal || isNaN(new Date(lastResetVal).getTime())) {
                adapter.log.info(
                    '[statisticsHelperMonth] Kein gültiges Reset-Datum vorhanden – Initialisierung ohne Reset.',
                );
                await adapter.setStateAsync('analytics.statistics.temperature.month.last_reset', {
                    val: now.toISOString(),
                    ack: true,
                });
                return; // kein Reset durchführen
            }

            const lastResetMonth = new Date(lastResetVal).getMonth();
            const currentMonth = now.getMonth();
            if (lastResetMonth === currentMonth) {
                adapter.log.info('[statisticsHelperMonth] Monatsreset übersprungen – gleicher Monat erkannt.');
                return; // kein Reset, wenn gleicher Monat
            }
        } catch (err) {
            adapter.log.warn(`[statisticsHelperMonth] Fehler bei Prüfung des Reset-Datums: ${err.message}`);
        }
