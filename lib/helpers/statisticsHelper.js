...
const statisticsHelper = {
    ...
    async _createTemperatureStatistics() {
        ...
        for (const sensor of this.sensors) {
            ...
            if (!isActive) {
                await adapter.setStateAsync(summaryJsonPath, { val: JSON.stringify({ status: 'kein Sensor aktiv' }), ack: true });
                await adapter.setStateAsync(summaryHtmlPath, { val: '<div style="color:gray;">kein Sensor aktiv</div>', ack: true });
                continue;
            }
                ...
                if (!obj) {
                    await adapter.setObjectNotExistsAsync(fullPath, {
                        ...
                        common: {
                            ...
                            role: def.id.includes('time') ? 'value.time' : def.id.includes('temp') ? 'value.temperature' : 'value',
                            read: true,
                            write: false,
                            persist: true,
                        },
    ...
    async _processTemperatureChange(sensorId, newValue) {
        ...
        await adapter.setStateAsync(
            `${basePath}.summary_html`,
            { val: `<div><b>Min:</b> ${newMin} °C / <b>Max:</b> ${newMax} °C / <b>Ø:</b> ${Math.round(newAvg * 100) / 100} °C</div>`, ack: true },
        );
    ...
    async _updateOverallSummary() {
        ...
        for (const sensor of this.sensors) {
            ...
            const min = (await adapter.getStateAsync(`analytics.statistics.temperature.today.${sensor.id}.temp_min`))?.val;
            const max = (await adapter.getStateAsync(`analytics.statistics.temperature.today.${sensor.id}.temp_max`))?.val;
            const avg = (await adapter.getStateAsync(`analytics.statistics.temperature.today.${sensor.id}.temp_avg`))?.val;

            allData.push({ name: sensor.name, min, max, avg });
        }

        await adapter.setStateAsync('analytics.statistics.temperature.today.outputs.summary_all_json', { val: JSON.stringify(allData), ack: true });
        ...
        await adapter.setStateAsync('analytics.statistics.temperature.today.outputs.summary_all_html', { val: html, ack: true });
    },
    ...
    async _scheduleMidnightReset() {
        ...
        adapter.log.debug(`statisticsHelper: Mitternacht-Reset geplant in ${Math.round(msUntilMidnight / 60000)} Minuten.`);
    },
    ...
    async _resetDailyTemperatureStats() {
        ...
        for (const sensor of this.sensors) {
            ...
            for (const state of stateList) {
                ...
                let defValue = null;
                if (state.includes('time')) defValue = '';
                if (state === 'data_points_count') defValue = 0;
                if (state === 'last_update') defValue = resetDate;
                await adapter.setStateAsync(fullPath, { val: defValue, ack: true });
            }

            await adapter.setStateAsync(summaryJsonPath, { val: JSON.stringify({ date_reset: resetDate, status: 'Tageswerte zurückgesetzt' }), ack: true });
            await adapter.setStateAsync(summaryHtmlPath, { val: `<div style="color:gray;">Tageswerte zurückgesetzt (${resetDate})</div>`, ack: true });
        }

        await adapter.setStateAsync('analytics.statistics.temperature.today.outputs.summary_all_json', { val: '{}', ack: true });
        await adapter.setStateAsync('analytics.statistics.temperature.today.outputs.summary_all_html', { val: '', ack: true });
