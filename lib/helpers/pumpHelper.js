...
const pumpHelper = {
  ...
  init(adapter) {
    ...
    this.currentPowerId =
      (adapter.config.pump_current_power_id || "").trim() || null;
    ...
    if (this.currentPowerId) {
      ...
      this.adapter
        ...
        .catch(() => {
          this.adapter.log.info(
            `[pumpHelper] Überwache Leistung von ${this.currentPowerId}`,
          );
        });
    } else {
      this.adapter.log.info(
        "[pumpHelper] Keine Objekt-ID für aktuelle Leistung konfiguriert",
      );
    }
    ...
    if (this.deviceId) {
      ...
      this.adapter.log.info(
        `[pumpHelper] Überwache Steckdose: ${this.deviceId}`,
      );
    } else {
      this.adapter.log.info(
        "[pumpHelper] Keine Objekt-ID für Pumpen-Steckdose konfiguriert",
      );
    }
    ...
    this._updateStatus().catch((err) =>
      this.adapter.log.warn(
        `[pumpHelper] Initiales Status-Update fehlgeschlagen: ${err.message}`,
      ),
    );
  },
  ...
  async _updateStatus() {
    try {
      const mode =
        (await this.adapter.getStateAsync("pump.mode"))?.val || "unknown";
      const active = (await this.adapter.getStateAsync("pump.pump_switch"))
        ?.val;
      ...
      this.adapter.log.warn(
        `[pumpHelper] Fehler beim Setzen von pump.status: ${err.message}`,
      );
    }
  },
  ...
  async _checkErrorConditions() {
    try {
      ...
      const power = this._parseNumber(
        (await this.adapter.getStateAsync("pump.current_power"))?.val,
      );
      const maxWatt = this._parseNumber(
        (await this.adapter.getStateAsync("pump.pump_max_watt"))?.val,
      );
      ...
      this.adapter.log.warn(
        `[pumpHelper] Fehler bei Error-Check: ${err.message}`,
      );
    }
  },
