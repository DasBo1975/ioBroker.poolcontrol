...
const pumpHelper2 = {
    adapter: null,
    lastKnownFlow: 0, // NEU: merkt sich den letzten gültigen Durchflusswert
    ...
    async handleStateChange(id, state) {
        ...
        if (id.endsWith('pump.pump_switch')) {
            ...
            if (!pumpOn) {
                // NEU (minimal): Sicherstellen, dass wir den letzten echten Flowwert erwischen,
                // bevor andere Events (Power=0) reinkommen.
                const currentFlowImmediate = await this._getNumber('pump.live.flow_current_lh');
                if (currentFlowImmediate > 0) {
                    this.lastKnownFlow = currentFlowImmediate;
                }
    ...
    async _updateLiveValues() {
        try {
            ...
            // NEU: letzten gültigen Wert merken
            if (flowCurrentLh > 0) {
