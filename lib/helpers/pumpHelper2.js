...
const pumpHelper2 = {
    ...
    async handleStateChange(id, state) {
        ...
        // Pumpenstatus-Änderung → letzten Durchflusswert sichern (richtiger Zeitpunkt)
        if (id.endsWith('pump.pump_switch')) {
            const pumpOn = state.val === true;

            if (!pumpOn) {
                // FIX: Den aktuellen Flow sofort sichern, bevor Power auf 0 fällt
                const flowBeforeStop = await this._getNumber('pump.live.flow_current_lh');
                if (flowBeforeStop > 0) {
                    this.lastKnownFlow = flowBeforeStop;
                    this.adapter.log.debug(`[pumpHelper2] FIX: Durchfluss vor Stopp gesichert: ${flowBeforeStop} l/h`);
                }

                // Danach Wert persistieren
    ...
    async _storeLastFlowValue() {
        try {
            const valueToStore = this.lastKnownFlow || 0;
            await this._setIfChanged('pump.live.last_flow_lh', valueToStore);
            this.adapter.log.debug(`[pumpHelper2] Letzter Durchflusswert gespeichert: ${valueToStore} l/h`);
        } catch (err) {
