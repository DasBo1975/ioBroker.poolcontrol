...
const consumptionHelper = {
    ...
    async handleStateChange(id, state) {
        if (!state) return;
        if (id !== this.energyId) return;

        const totalNowRaw = Number(state.val);
        if (!Number.isFinite(totalNowRaw)) return;
    ...
    async _updateConsumption(totalNowRaw) {
        try {
            ...
            await this.adapter.setStateAsync("consumption.week_kwh",  { val: Number(values.week.toFixed(3)),  ack: true });
            await this.adapter.setStateAsync("consumption.month_kwh", { val: Number(values.month.toFixed(3)), ack: true });
            await this.adapter.setStateAsync("consumption.year_kwh",  { val: Number(values.year.toFixed(3)),  ack: true });

            // Kosten berechnen
            if (this.price > 0) {
                await this.adapter.setStateAsync("costs.day_eur",   { val: Number((values.day   * this.price).toFixed(2)),   ack: true });
                await this.adapter.setStateAsync("costs.week_eur",  { val: Number((values.week  * this.price).toFixed(2)),  ack: true });
                await this.adapter.setStateAsync("costs.month_eur", { val: Number((values.month * this.price).toFixed(2)), ack: true });
                await this.adapter.setStateAsync("costs.year_eur",  { val: Number((values.year  * this.price).toFixed(2)),  ack: true });
                await this.adapter.setStateAsync("costs.total_eur", { val: Number((totalNow     * this.price).toFixed(2)),  ack: true });
            }
