...
const consumptionHelper = {
  ...
  init(adapter) {
    ...
    if (this.energyId) {
      ...
      adapter.log.info(
        `[consumptionHelper] Überwache externen kWh-Zähler: ${this.energyId}`,
      );
    } else {
      adapter.log.info(
        "[consumptionHelper] Kein externer kWh-Zähler konfiguriert → Verbrauchslogik inaktiv.",
      );
    }
  ...
  async _updateConsumption(totalNowRaw) {
    try {
      // Offset laden
      const offset =
        (await this.adapter.getStateAsync("consumption.offset_kwh"))?.val || 0;
      const last =
        (await this.adapter.getStateAsync("consumption.last_total_kwh"))?.val ||
        0;
      ...
      if (totalNowRaw < last) {
        this.adapter.log.warn(
          "[consumptionHelper] Zähler-Reset erkannt → Offset wird angepasst",
        );
      ...
      this.adapter.log.warn(
        `[consumptionHelper] Fehler bei Verbrauchsupdate: ${err.message}`,
      );
    }
  },
  ...
  async _loadBaselines(totalNow) {
    ...
    const week = (await this.adapter.getStateAsync("consumption.week_kwh"))
      ?.val;
    const month = (await this.adapter.getStateAsync("consumption.month_kwh"))
      ?.val;
    const year = (await this.adapter.getStateAsync("consumption.year_kwh"))
      ?.val;
    ...
    this.adapter.log.info(
      `[consumptionHelper] Baselines geladen: ${JSON.stringify(this.baselines)}`,
    );
  },
